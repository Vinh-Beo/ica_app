...
class _ClientScreenState extends State<ClientScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: EdgeInsets.fromLTRB(16, 16, 16, 24), children: [
      Row(children: [
        ...
        GestureDetector(
          ...
          child: AnimatedContainer(duration: Duration(milliseconds: 150), padding: EdgeInsets.symmetric(horizontal: 14, vertical: 8),
            decoration: BoxDecoration(color: _showAdd ? Color(0xFFFEF2F2) : context.p.navy, borderRadius: BorderRadius.circular(20)),
        ...
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(padding: EdgeInsets.symmetric(horizontal: 10), decoration: BoxDecoration(color: context.p.bg, borderRadius: BorderRadius.circular(10), border: Border.all(color: context.p.border, width: 1.5)),
              child: DropdownButton<String>(value: _newType, isExpanded: true, underline: SizedBox(), dropdownColor: context.p.surface,
                items: kCustomerTypes.map((t) => DropdownMenuItem(value: t, child: Row(children: [Icon(kTypeIcon[t]?.icon ?? Icons.store_rounded, size: 14, color: kTypeIcon[t]?.color ?? context.p.text2), SizedBox(width: 6), Text(t, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: context.p.textMain))]))).toList(),
                onChanged: (v) => setState(() => _newType = v!))),
          ])),
          SizedBox(width: 10),
          SizedBox(width: 90, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            FieldLabel(s.coeff),
            OceanInput(hint: '1.20', keyboardType: TextInputType.numberWithOptions(decimal: true), onChanged: (v) => _newCoeff = v),
          ])),
        ]),
        SizedBox(height: 12),
        GestureDetector(
          ...
          child: Container(height: 44, decoration: BoxDecoration(color: context.p.teal, borderRadius: BorderRadius.circular(22)),
              child: Center(child: Text(s.addCust, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 14)))),
        ),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        // ── header ──
        Padding(padding: EdgeInsets.fromLTRB(14, 14, 14, 0), child: Row(children: [
              ...
              if (_uploading)
                ...
                Positioned(right: 0, bottom: 0,
                  child: Container(width: 16, height: 16,
                    ...
                    child: Icon(Icons.camera_alt_rounded, size: 10, color: Colors.white))),
            ]),
          ),
          SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(margin: EdgeInsets.only(top: 3), padding: EdgeInsets.symmetric(horizontal: 8, vertical: 1),
              decoration: BoxDecoration(color: Color(0xFFE0F7FA), borderRadius: BorderRadius.circular(12)),
          ...
          Container(padding: EdgeInsets.symmetric(horizontal: 11, vertical: 5),
            decoration: BoxDecoration(color: Color(0xFFE0F7FA), borderRadius: BorderRadius.circular(10)),
        ...
        // ── price table ──
        Padding(padding: EdgeInsets.fromLTRB(14, 10, 14, 10), child: Column(children: [
          Container(decoration: BoxDecoration(border: Border.all(color: context.p.border), borderRadius: BorderRadius.circular(12)), child: Column(children: [
            Container(padding: EdgeInsets.symmetric(horizontal: 10, vertical: 6),
              decoration: BoxDecoration(color: context.p.bg, borderRadius: BorderRadius.vertical(top: Radius.circular(11))),
              child: Row(children: [
            ...
            if (pricedSf.isEmpty)
              Padding(padding: EdgeInsets.all(10), child: Text(s.noBaseTbl, style: TextStyle(fontSize: 11, color: Color(0xFFCBD5E1)), textAlign: TextAlign.center))
            else ...pricedSf.asMap().entries.map((entry) => Container(
              decoration: BoxDecoration(border: entry.key > 0 ? Border(top: BorderSide(color: Color(0xFFF4F7F8))) : null),
              padding: EdgeInsets.symmetric(horizontal: 10, vertical: 7),
              child: Row(children: [
                Expanded(child: Text('${entry.value.icon} ${entry.value.name}', style: TextStyle(fontSize: 11, color: Color(0xFF475569), fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis)),
          ...
          Container(padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(color: Color(0xFFE0F7FA), borderRadius: BorderRadius.circular(10)),
            child: Row(children: [
              Expanded(child: Text(s.sellTotal, style: TextStyle(fontSize: 11, color: Color(0xFF0E7C8C), fontWeight: FontWeight.w600))),
              Text('${fmt(totalSell)} đ', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: context.p.teal)),
            ])),
        ])),

        // ── delete ──
        Padding(padding: EdgeInsets.fromLTRB(14, 0, 14, 12), child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          if (_isConfirmDel) ...[
            Text(s.deleteQ, style: TextStyle(fontSize: 11, color: Color(0xFFDC2626), fontWeight: FontWeight.w700)),
            SizedBox(width: 7),
            GestureDetector(
              ...
              child: Container(padding: EdgeInsets.symmetric(horizontal: 12, vertical: 5),
                  decoration: BoxDecoration(color: Color(0xFFDC2626), borderRadius: BorderRadius.circular(16)),
                  child: Text(s.confirm, style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700)))),
            SizedBox(width: 6),
            ...
            child: Container(padding: EdgeInsets.symmetric(horizontal: 12, vertical: 6),
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: Color(0xFFFECACA), width: 1.5)),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.delete_outline_rounded, size: 13, color: Color(0xFFDC2626)),
                SizedBox(width: 4),
                Text(s.delete, style: TextStyle(color: Color(0xFFDC2626), fontSize: 11, fontWeight: FontWeight.w700)),
              ]))),
        ])),
      ]),
    );
    ...
    try {
      ...
      messenger.showSnackBar(SnackBar(
        content: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(width: 8),
          Text(s.updToast, style: TextStyle(color: Colors.white, fontSize: 13)),
        ]),
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: Container(width: 40, height: 4, margin: EdgeInsets.only(bottom: 14),
          ...
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(padding: EdgeInsets.symmetric(horizontal: 10),
                ...
                child: DropdownButton<String>(
                  value: _type, isExpanded: true, underline: SizedBox(), dropdownColor: context.p.surface,
            ...
            SizedBox(width: 90, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              FieldLabel(s.coeff),
              OceanInput(hint: '1.20', controller: _coeffCtrl, keyboardType: TextInputType.numberWithOptions(decimal: true)),
            ])),
          ]),
          SizedBox(height: 10),
          ...
          GestureDetector(
            ...
            child: Container(height: 48,
              ...
              child: Center(child: _saving
                  ...
                  : Text(s.saveChanges, style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 15)))),
          ),
        ]),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        AppIcon(icon: Icons.inventory_2_rounded, color: Color(0xFF7C3AED), size: 72, iconSize: 34),
        SizedBox(height: 14),
