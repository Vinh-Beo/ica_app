...
class _ImportExportScreenState extends State<ImportExportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Container(color: context.p.bg, padding: const EdgeInsets.fromLTRB(16,12,16,0), child: Column(children: [
        ...
        Row(children: [
          Container(padding: const EdgeInsets.all(3), decoration: BoxDecoration(color: context.p.surface, borderRadius: BorderRadius.circular(22), border: Border.all(color: context.p.border, width: 1.5)), child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 10), decoration: BoxDecoration(color: context.p.bg, borderRadius: BorderRadius.circular(10), border: Border.all(color: context.p.border, width: 1.5)),
          child: DropdownButton<String>(
            value: _sfId, isExpanded: true, underline: const SizedBox(), dropdownColor: context.p.surface,
  ...
  List<Widget> _buildTonKho(AppState state) {
    ...
    if (visible.isEmpty) {
      return [Center(child: Padding(padding: const EdgeInsets.only(top: 50), child: Column(children: [
        AppIcon(icon: Icons.search_off_rounded, color: context.p.textMuted, size: 60, iconSize: 28), const SizedBox(height: 10),
    ...
    return visible.map((sf) {
      ...
      final tonColor = ton > 0 ? const Color(0xFF15803D) : ton < 0 ? const Color(0xFFDC2626) : context.p.textMuted;
      final tonBg    = ton > 0 ? const Color(0xFFDCFCE7) : ton < 0 ? const Color(0xFFFEE2E2) : context.p.surface2;
      return Container(
        ...
        child: Column(children: [
          Row(children: [
            Container(width: 42, height: 42, decoration: BoxDecoration(color: const Color(0xFFEEF6F7), borderRadius: BorderRadius.circular(13)),
                child: Center(child: Text(sf.icon, style: const TextStyle(fontSize: 21)))),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: tonBg, borderRadius: BorderRadius.circular(10)),
  ...
  List<Widget> _buildGiaoDich(BuildContext context, AppState state, List<InventoryEntry> periodEntries) {
    ...
    final s = AppStrings.of(context);
    if (grouped.isEmpty) return [Center(child: Padding(padding: const EdgeInsets.only(top: 60), child: Column(children: [AppIcon(icon: hasFilter ? Icons.search_off_rounded : Icons.inventory_2_rounded, color: context.p.textMuted, size: 60, iconSize: 28), const SizedBox(height: 10), Text(hasFilter ? s.noTxFound : s.noTx, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: context.p.textMuted))])))];
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        ...sfIds.map((id) {
          ...
          return Container(
            ...
            child: Row(children: [
              Text(sf.icon, style: const TextStyle(fontSize: 16)), const SizedBox(width: 8),
              Expanded(child: Text(sf.name, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Colors.white), overflow: TextOverflow.ellipsis)),
              _SumChip('📥 ${f(n)}'), const SizedBox(width: 5), _SumChip('📤 ${f(x)}'), const SizedBox(width: 8),
              SizedBox(width: 60, child: Text('${net > 0 ? "+" : ""}${f(net)} ${sf.unit}', textAlign: TextAlign.right, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Colors.white))),
            ]),
          );
        }),
      ]),
      ...
      children: [
        ...
        ...entry.value.map((e) {
          ...
          return Container(
            ...
            child: Row(children: [
              Container(width: 42, height: 42, decoration: BoxDecoration(color: bgIcon, borderRadius: BorderRadius.circular(13)), child: Center(child: Text(sf.icon, style: const TextStyle(fontSize: 20)))),
              const SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Flexible(child: Text(sf.name, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: context.p.textMain))),
                  const SizedBox(width: 6),
                  Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2), decoration: BoxDecoration(color: bgIcon, borderRadius: BorderRadius.circular(9)), child: Text(badgeTxt, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: qtyColor))),
                ]),
                ...
                if (_confirmDelId == e.id) ...[
                  ...
                  Row(children: [
                    GestureDetector(onTap: () { state.deleteInventoryEntry(e.id); setState(() => _confirmDelId = null); showToast(context, s.deleteDebtToast); },
                        child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFDC2626), borderRadius: BorderRadius.circular(8)), child: Text(s.delete, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700)))),
                    const SizedBox(width: 4),
                    GestureDetector(onTap: () => setState(() => _confirmDelId = null),
                        child: Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 4), decoration: BoxDecoration(color: context.p.bg, borderRadius: BorderRadius.circular(8)), child: Text(s.cancel, style: TextStyle(fontSize: 10, color: context.p.text2)))),
                  ]),
                ] else ...[
                  const SizedBox(height: 4),
                  GestureDetector(onTap: () => setState(() => _confirmDelId = e.id),
                      child: Container(width: 26, height: 26, decoration: BoxDecoration(color: const Color(0xFFFEF2F2), borderRadius: BorderRadius.circular(8)), child: const Center(child: Icon(Icons.delete_outline_rounded, size: 14, color: Color(0xFFDC2626))))),
                ],
              ]),
            ]),
          );
...
class _NXStat extends StatelessWidget {
  ...
  const _NXStat(this.title, this.value, this.sub, this.colors);
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(title.toUpperCase(), style: const TextStyle(fontSize: 9, color: Colors.white70, fontWeight: FontWeight.w700, letterSpacing: 0.8)),
...
class _InvStatBox extends StatelessWidget {
  ...
  final int count;
  const _InvStatBox(this.label, this.qty, this.unit, this.color, this.bg, this.count);
...
class _ViewBtn extends StatelessWidget {
  ...
  const _ViewBtn(this.label, this.key_, this.active, this.onTap);
...
class _TypeBtn extends StatelessWidget {
  final String label, key_, active;
  final VoidCallback onTap;
  const _TypeBtn(this.label, this.key_, this.active, this.onTap);
